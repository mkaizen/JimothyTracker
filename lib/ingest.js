...
export async function ingestPosts(posts) {
  ...
  for (const post of posts) {
    const r = await resolvePost(post);
    if (r.status === 'skip') {
      skipped++;
      details.push({ id: post.id, status: 'skipped', reason: r.reason });
      continue;
    }
    const rowId = addSighting(r.sighting);
    if (rowId) {
      added++;
      details.push({ id: post.id, status: r.sighting.lat != null ? 'added' : 'added (feed-only)', rowId });
    } else {
      skipped++;
...
/**
 * Resolve one raw post into a storable sighting, WITHOUT touching the database.
 * This is the geocoding-heavy step; it runs wherever the scraper runs (e.g. a
 * home PC) so the server that finally stores the result stays fast.
 * @param {RawPost} post
 * @returns {Promise<{status:'ready', sighting:object} | {status:'skip', reason:string}>}
 */
export async function resolvePost(post) {
  let loc = null;
  ...
  if (!mapped && !post.photoUrl) {
    return { status: 'skip', reason: loc ? 'outside-seattle' : 'no-location' };
  }
...
/**
 * Resolve a batch of posts into storable sightings for POSTing to a remote
 * server's /api/ingest endpoint. Does no DB writes.
 * @param {RawPost[]} posts
 * @returns {Promise<{sightings:object[], skipped:number, details:Array}>}
 */
export async function resolvePosts(posts) {
  const sightings = [];
  const details = [];
  let skipped = 0;
  for (const post of posts) {
    ...
    if (r.status === 'skip') {
      ...
      sightings.push(r.sighting);
      details.push({ id: post.id, status: 'resolved' });
    }
  }
  return { sightings, skipped, details };
}
